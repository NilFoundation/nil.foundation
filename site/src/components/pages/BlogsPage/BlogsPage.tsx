...
import {
	...
	MappedCategory,
	MappedTag,
} from "src/strapi/types/entities";
				...
				<div className={s.inner}>
									...
									<div className={s.buttonsWrapper}>
										...
										{data.categories.map((button) => (
											<Button
												key={button.slug}
												onClick={() =>
													router.push(`/blog/category/${button.slug}`)
												}
												className={cx(s.filterButtons, {
					...
					<div className={cx(s.content, s.centeredItems)}>
						{data.posts && data.posts.length > 0 ? (
							data.posts.map((post) => (
								<PostCard key={post.id} className={s.blogPost} post={post} />
							))
						) : (
					...
					<FooterAnimationSection
						items={
							isMobile ? blogsPageFooterMobileItemData : blogsPageFooterItemData
						}
