...
export const Community = () => {
    ...
    const tiles = useMemo(() => Array.from({length: rows * columns}, (_, i) => i).map(i => {
        const x = i % columns;
        ...
        if (maybeHole !== null){
            ...
            if (x >= left && x <= right){
                return (
                  <a key={i} className={classNames(s.community__tile, s.community__tile_invisible, s.tile)} href={landingPageData.joinLink} aria-label="Empty community space">
        ...
        return (
            <a key={i} className={classNames(s.community__tile, s.tile)} href={landingPageData.joinLink} aria-label={realAvatarPosition[i] ? "Community member tile" : "Available community space"}>
              {realAvatarPosition[i] && (
                <img 
                  src={realAvatarPosition[i]}
                  alt="Community member"
                  className={s.tile__avatar}
                />
              )}
            </a>
        )
    ...
    return (
        <div className={commonStyle.wrap}>
            <div className={s.info}>
                ...
                <p className={s.info__description}>zkSharding combines a sharded architecture with zero knowledge security to enable horizontal scaling across a unified network of shards.</p>
            </div>
            <div className={s.blocks}>
                ...
                <a className={classNames(s.block, s.block__middle, s.block__middle_left)} href='https://explore.nil.foundation/sandbox'>
                ...
                <a className={classNames(s.block, s.block__middle, s.block__middle_right)} href={landingPageData.joinLink}>
                    ...
                    <p className={s.block__description}>Join our Testnet and set up your environment with =nil; CLI and RPC node.</p>
                </a>
            ...
            <div className={s.community}>
                ...
                {isMobile && (<LimitedButton arrow primary className={s.community__button} href={landingPageData.anounceLink}>Join the community</LimitedButton>)}
            </div>
            <div className={s.join}>
                ...
                <div className={s.join__block}>
                    ...
                    <p className={s.join__description}>Your feedback is crucial. We&apos;re building a network to serve ambitious, visionary developers and we&apos;ll help you build apps that can serve millions.</p>
                    <div className={s.join__avatars}>
                        {supportImages.map((avatar, index) => (
                            <a className={s.tile} key={index} href={landingPageData.anounceLink}>
                                <img key={index} src={avatar} alt="Community Avatar" className={classNames(s.join__avatar, s.tile__avatar)} />
                            </a>
                        ))}
                    ...
                    <LimitedButton arrow primary className={s.join__button} href={landingPageData.anounceLink}>Get in touch</LimitedButton>
                </div>
            </div>
        </div>
    )
