...
export const Intro = () => {
    ...
    return (
        ...
        <div className={s.intro}>
                ...
                <p className={s.descr}>One network for all<br/><span className={s.descr__gray}>your projects</span></p>
                <div className={s.buttons}>
                        <LimitedButton arrow href={landingPageData.docsLink}>Read docs</LimitedButton>
                        <LimitedButton primary href={landingPageData.joinLink}>Join testnet</LimitedButton>
                </div>
        </div>
        <div className={classNames(commonStyles.wrap, s.section__wrap)}><div className={s.section}>
            {items.map((_, index) => {
                return (<div className={classNames(s.item__top, indexToStyle('item__top', index))} key={index}>
                </div>);
            })}
            {items.map((item, index) => {
                return (<div className={classNames(s.item, indexToStyle('item__middle', index))} key={index}>
                    <p className={s.item__title}>{item.title}</p>
                    <p className={s.item__description}>{item.description}{' '}<a className={s.item__link} href={item.link}>Learn&nbsp;more.</a></p>
                </div>);
            })}
            {items.map((_, index) => {
                return (<div className={classNames(s.item__bottom, indexToStyle('item__bottom', index))} key={index}>
                </div>);
            })}
        </div></div>
        <div className={classNames(s.enhance, commonStyles.wrap)}>
            ...
            <div className={s.enhance__info}>=nil; blends the scalability of appchains with the composability of a monolithic chain to enable super scale apps. <a href="https://nil.foundation/blog/post/nil_zkSharding">Learn about zkSharding.</a></div>
        </div>
        {isMobile ? <Line /> : null}
        </>
    )
