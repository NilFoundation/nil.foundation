...
import { Modal } from '@nilfoundation/ui-kit'
...
export const Uniswap = () => {
  const [sellAmount, buyAmount, buyCurrency, isLoading, swapError, transactions, stage, quotePending, isMobilePopup] =
    useUnit([
      $sellAmount,
      $buyAmount,
      $buyCurrency,
          ...
          <AnimatePresence>
            {transactions.map((tx, index) => (
              ...
              >
                ...
                <a href={`https://explore.nil.foundation/tx/${tx.Tx}`} target="_blank">
                  {txView(tx.Tx)}
                </a>{' '}
                {!tx.External ? (
                  index === 1 ? (
  ...
  return (
    <div className={commonStyle.wrap}>
      ...
      <div className={classNames(s.blocks, stage !== 'idle' ? s.blocks__swap : '')}>
        ...
        {!isMobile && animationNode}
        {isMobile && isMobilePopup && (
          <Modal
            isOpen
            onClose={() => hidePage()}
            overrides={{
          ...
          >
            <div className={s.mobileAnimation}>{animationNode}</div>
          </Modal>
        )}
