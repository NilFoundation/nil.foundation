...
import { $buyAmount, $buyCurrency, $sellAmount, $sendedTransactionHash, $swapError, $transactions, infoFx, quoteFx, setBuyCurrency, setSellAmount, swap, swapFx, tick, UniswapTransactionInfo } from './model';
...
infoFx.use(async ({ hash }) => {
    ...
    const outputCount = txs.reduce((acc, tx) => { return acc + (tx.OutTxs && tx.OutTxs.length || 0);  }, 0);
